...
package="github.com/tinyci/ci-runners"

dirs=$(go list ./... | sed -e "s!${package}!.!g" | grep -vE '^.$' | grep -v ./vendor)
files=$(find . -type f -name '*.go' 2>/dev/null | grep -vE '^.$' | grep -v ./vendor | grep -v ./gen)
...
echo "Running govet..."
set +e
out=$(go vet -composites=false ${dirs} 2>&1 | grep -v vendor | grep -v gen)
set -e
...
set +e
out=$(gocyclo -over 15 . | grep -v vendor | grep -v gen)
set -e
if [ "`echo \"${out}\" | sed '/^$/d' | wc -l`" -gt 0 ]
then
...
set +e
out=$(misspell -locale US -error -i exportfs ${dirs} | grep -vE '^(vendor|gen)')
set -e
if [ "`echo \"${out}\" | sed '/^$/d' | wc -l`" -gt 0 ]
then
